...
<p align="center">
  <picture>
    ...
    <img alt="Trendchaser 시스템 아키텍처 - enabled 소스 101개와 X following fan-out을 fetch, deduplicate, enrich, score, write, validate, publish, deliver gate로 통과시킨 뒤 main과 claude/brief-stream 두 브랜치에 저장하고 Telegram, KakaoTalk, Obsidian, graph layer, 선택적 Notion archive로 보낸다." src="./assets/architecture.svg" width="100%">
  </picture>
</p>

| # | 단계 | 하는 일 |
|--:|---|---|
| 1 | **Fetch** | enabled 소스 101개 + X following fan-out 병렬 폴링; 각 소스 best-effort, 장애 격리 |
...
PRISMA-Codex 확장(2026-07-07) 기준 `sources.yaml`에는 **설정 소스 116개**, 그중 **enabled 소스 채널 101개**가 있다. 기존 Claude Routine 표면은 GitHub Actions + Codex auth로 이전했다. X/watch proxy는 별도 계층에 남겨 토큰, 계정 inventory, follow-list 내용은 이 공개 리포트에 노출하지 않는다. 비활성 경로는 설정에 남아 있지만 집계에서 제외한다: 사망한 피드, Gmail newsletter, 불안정한 YouTube RSS, 저신호 X search, 실험용 handle.
...
| **enabled 합계** | **101** | private X account inventory는 공개 집계에서 제외 |
...
| 날짜 | 노트 | 한 줄 요약 |
|---|---|---|
| 2026-07-07 | [`releases/2026-07-07.ko.md`](./releases/2026-07-07.ko.md) · [English](./releases/2026-07-07.md) | PRISMA-Codex 전환 - GitHub Actions + Codex auth, enabled 채널 101개, 학술/소스 대확장, 교차 소스 확인, 정시 슬롯 guard 반영 |
