...
PRISMA-Codex가 GitHub Actions와 Codex auth를 통해 매 슬롯 전체 파이프라인을 서버 사이드에서 실행한다 — 운영자의 컴퓨터는 꺼져 있어도 된다. 현재 루프는 여섯 macro 단계에 검증·export·publish gate가 붙은 형태다: fetch · deduplicate · enrich+shortlist · score+editorial review · write+postcheck · publish+deliver.

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="./assets/architecture-dark.svg">
    <img alt="Trendchaser 시스템 아키텍처 - X following fan-out을 계정별로 센 실효 enabled 채널 184개를 fetch, deduplicate, enrich, score, write, validate, publish, deliver gate로 통과시킨 뒤 main과 claude/brief-stream 두 브랜치에 저장하고 Telegram, KakaoTalk, Obsidian, graph layer, 선택적 Notion archive로 보낸다." src="./assets/architecture.svg" width="100%">
  </picture>
</p>

| # | 단계 | 하는 일 |
|--:|---|---|
| 1 | **Fetch** | X following fan-out을 계정별로 세면 실효 enabled 채널 184개 병렬 폴링; 설정상 enabled entry는 101개; 각 소스 best-effort, 장애 격리 |
...
Two-branch 게시 전략: `state/seen.json`은 `main`에 (다음 실행이 최신 dedup 상태를 봄), 브리프·raw snapshot·Obsidian note·graph layer·`briefs.json`은 `claude/brief-stream`에 누적되어 Obsidian과 웹사이트가 그 브랜치를 follow.
...
enabled fetcher 계열 20개(`rss`/Atom, `arxiv`, `semantic_scholar`, `openalex`, `paperswithcode`, `conference_schedule`, `proceedings_page`, `hn`, `hn_search`, `hf_papers`, `hf_models` [trending/new mode], `github_trending`, `github_search`, `sitemap`, `hf_posts`, `bluesky_search`, `mastodon`, `reddit_rss`, `vercel_x`, `vercel_watch`)를 병렬 수집. 소스별 `RateLimiter` (arxiv 3.0초, GitHub API 1.0초, 그 외 0.2초 default). `ThreadPoolExecutor(max_workers=5)`. 각 fetcher는 `list[Item]`을 반환하고 raise 금지 — 실패는 orchestrator의 `failures[]` 배열로 수집.
...
3. **브리프 history dedup** — Step 1에서 `origin/claude/brief-stream`과 최근 stray legacy `claude/*` branch를 worktree에 overlay한 뒤, Codex가 최근 14일치 `briefs/*.md`를 URL/title overlap과 의미 기반 content-entity match로 스캔. URL/title match 또는 Jaccard ≥ 0.6이면 drop, URL이 달라도 회사 + 제품 + 버전/사건어가 같은 동일 사건이면 drop.
4. **교차 소스 entity 확인** — DOI, arXiv ID, OpenReview ID, GitHub repo, Hugging Face model ID, 모델/릴리즈명, 정규화 title fallback을 `cross_confirm_key`로 묶는다. 독립 entity 2곳 이상에서 같은 항목이 잡히면 `cross_confirm_count` / `cross_confirm_sources` metadata와 ranking boost를 받지만, freshness·원본 URL 무결성·환각 검증 gate는 우회하지 못한다.
...
Codex가 `profile.md`(큐레이션 프로필)를 읽고 항목별 합성 점수 계산. 현재도 **6-axis score**를 쓰지만, 2026-05-11 이후 주변 gate가 더 엄격해졌다:
...
작성 직전 Step 8.5에서 **환각/source 검증 루프**가 돈다. 소속·저자, 약자 풀이, 수치, 모델/repo 식별자, 직접 인용 같은 load-bearing claim은 가능하면 원문 URL과 대조한다. 실패한 claim은 Codex가 제거/완화하거나, 핵심 사실이면 item을 drop하고 shortlist에서 대체 후보를 승격한다.
...
PRISMA-Codex 확장(2026-07-07) 기준 `sources.yaml`에는 **설정 소스 116개**, **enabled 설정 entry 101개**, 그리고 private X follow-list를 계정별로 센 **실효 enabled 채널 184개**가 있다. 기존 Claude Routine 표면은 GitHub Actions + Codex auth로 이전했다. X/watch proxy는 별도 계층에 남겨 토큰, 계정 inventory, follow-list 내용은 이 공개 리포트에 노출하지 않는다. 비활성 경로는 설정에 남아 있지만 집계에서 제외한다: 사망한 피드, Gmail newsletter, 불안정한 YouTube RSS, 저신호 X search, 실험용 handle.

| 카테고리 | 채널 수 | 성격 |
|---|---:|---|
| RSS / Atom firehose | **43** | 1차 블로그, release atom, 큐레이터, 한국 dev blog, longform, aggregator |
| X via Vercel proxy | **95 effective** | private follow-list 84계정 + 직접 설정한 공식/고신호 handle 11개; 계정 inventory는 비공개 |
| Vercel watch | **17** | docs, leaderboard, 학회 페이지, RSS/API 없는 OpenReview venue page |
| 학술 API + 학회/proceedings | **13** | arXiv, Semantic Scholar, OpenAlex, Papers With Code, ICML/NeurIPS/ICLR/MLSys schedule, ACL/CVPR proceedings |
| Hugging Face native surface | **4** | HF Papers, HF Models new/trending, HF community posts |
| GitHub discovery | **3** | GitHub Trending + AI repo recent-push search |
...
| **실효 enabled 합계** | **184** | private X follow-list를 계정별로 세되 계정명은 공개하지 않음 |

Weight와 파라미터:

### PRISMA-Codex 확장

2026-07-07 패치는 collector를 소수의 고확신 피드 중심에서 더 넓은 AI trend radar로 바꿨다. 다만 선정 gate는 더 보수적으로 유지한다: 약한 소스는 후보 발견에는 쓰지만, 최종 승격은 recency, 원본 URL 무결성, 직접 권위 또는 교차 소스 확인을 통과해야 한다.

| 레이어 | 새로 강조된 소스 | Weight 범위 | 역할 |
|---|---|---:|---|
| Tier S 공식 / 학회 | `icml_2026_schedule`, `neurips_2026_schedule`, `iclr_2026_schedule`, `mlsys_2026_schedule`, 공식 모델 docs watch | 1.35-1.75 | schedule, keynote, paper list, 모델 문서 변경이 2차 요약으로 번지기 전 포착 |
| 학술 index | `semantic_scholar_latest_ai`, `openalex_latest_ai`, `paperswithcode_ai`, focused arXiv query | 1.05-1.15 | 특정 DB 하나를 진실로 믿지 않고 넓게 수집 |
| 구현/adoption smoke | `github_search_ai_repos`, GitHub Trending, HF Models/Posts | 0.9-1.4 | repo, weights, eval harness, demo가 움직이는 순간 포착 |
| 커뮤니티 early smoke | `reddit_machinelearning_new`, `reddit_localllama_new`, Bluesky, Mastodon, HN search | 0.6-1.3 | 발견용. 단독 권위는 낮게 두고 교차 확인 없으면 승격 억제 |
| Opaque-page watch | LMArena, Artificial Analysis, OpenRouter, COLM/CoRL/RSS/UAI/OpenReview page | 1.0-1.35 | RSS/API가 없는 고가치 페이지 변경 감시 |

### 학술 / 연구 확장

| ID | Type | Weight | 역할 |
|---|---|---:|---|
| `arxiv_llm_agents` | arXiv API | 1.15 | LLM/agent focused query. raw all-AI arXiv보다 좁아 가중치 높음 |
| `arxiv_multimodal_video` | arXiv API | 1.10 | multimodal, VLM, video generation |
| `arxiv_robotics_embodied` | arXiv API | 1.10 | robotics, embodied agent, action model |
| `semantic_scholar_latest_ai` | Semantic Scholar | 1.05 | arXiv 교차 확인 + metadata 보강 |
| `openalex_latest_ai` | OpenAlex | 1.05 | 독립 학술 index. Semantic Scholar와 같은 레이어 |
| `paperswithcode_ai` | Papers With Code | 1.15 | 논문 + 구현/adoption 신호. 일반 학술 index보다 위, 공식/학회 Tier S보다 아래 |
| `icml_2026_schedule` | Conference schedule | 1.75 | ICML 기간 최상위 학술 이벤트 stream |
| `neurips_2026_schedule` | Conference schedule | 1.70 | 최상위 ML conference schedule/program 변경 |
| `iclr_2026_schedule` | Conference schedule | 1.65 | representation learning conference surface |
...
| `watch_openreview_colm_2026` / `watch_openreview_corl_2026` / `watch_openreview_rss_2026` / `watch_openreview_uai_2026` | 1.05 | OpenReview API가 불안정할 때 HTML venue page fallback |
...
| `github_search_ai_repos` | GitHub Search | 1.1 | recent-push AI repo query. Trending보다 낮고, 저권위 social smoke보다 높음 |
...
| `semantic_scholar_latest_ai` | Semantic Scholar | 1.05 | 최신 AI paper index, optional API key |
| `openalex_latest_ai` | OpenAlex | 1.05 | 독립 학술 index, optional email/API key |
| `paperswithcode_ai` | Papers With Code | 1.15 | 구현이 붙은 paper discovery |
...
| `reddit_machinelearning_new` | Reddit RSS | 0.6 | r/MachineLearning/new early smoke. keyword-filtered, 교차 확인 없으면 저권위 |
| `reddit_localllama_new` | Reddit RSS | 0.6 | r/LocalLLaMA/new model/weights/quant smoke. cap + cross-confirm 의존 |
| `mastodon_hashtag_aiml` / `mastodon_hashtag_fosstodon` | Mastodon | 0.9 / 0.7 | hashtag mode; favorite/reblog thresholds |

### X 큐레이션 (제한적)

X 표면은 **실효 95개 채널**이다: private `x_my_following` 84계정과 직접 설정한 공식 lab / 고신호 researcher handle 11개를 함께 본다. `sources.yaml`에는 `x_my_following`이 비공개로 fan-out되므로 enabled `vercel_x` 설정 entry가 12개로 보인다. 정식 X API는 사용하지 않으며, 별도 자체 호스팅 Vercel relay를 거쳐 raw 텍스트만 가져온다. 공개 로그에는 성공/실패 상태만 남기고 토큰, 계정 inventory, follow-list 내용은 노출하지 않는다. 핵심 표면은 `x_my_following`이며, cron warmer lag로 fresh post가 누락되지 않도록 24h lookback을 둔다. 채널 회전은 코드 수정 없이 follow만 바꾸면 된다.
...
| ~~`thebrowser`~~ | Gmail Newsletter | — | Gmail connector auth 설정 전까지 비활성 |
| ~~`notboring`~~ | Gmail Newsletter | — | Gmail connector auth 설정 전까지 비활성 |
...
|---|---|---|---|
| ~~`youtube_ai`~~ | YouTube RSS | — | 2026-07-07 health probe에서 RSS endpoint 404/500 불안정 확인 후 비활성. 채널 페이지는 향후 안정 경로가 생기면 재도입 가능 |

**AI 분류 규칙** — `AI_SOURCES`에 들어간 source ID는 정의상 AI다: HF papers/models/posts, focused arXiv, Semantic Scholar, OpenAlex, Papers With Code, lab blog/watch, 주요 release atom, AI curator, alignment source, GitHub Trending/Search, HN AI search, Reddit AI community, conference/proceedings surface, 일부 한국 AI 신호. 그 외 source는 title + body excerpt가 `profile.md` AI Keywords와 강하게 매치될 때만 AI로 이동하고, 아니면 General dev-domain gate를 통과해야 한다.

---

## 점수화 공식

Codex가 점수화 단계에서 항목별로 계산. core score는 **6-axis composite**이다:
...
Resurfacing feed에서는 `published_at`이 진짜 story 시각이 아닐 수 있다. 큐레이션/index 피드(`hf_papers`, `hn_top`, `techmeme`, `lobsters_ai`, `semantic_scholar_latest_ai`, `openalex_latest_ai`, `paperswithcode_ai`)는 가능하면 24h cutoff에 `signal_at`을 쓰고, trending/search feed(`github_trending_*`, `github_search_ai_repos`, `hf_models_trending`)는 recency 점수에 artifact age 또는 push 시각을 사용해 오래된 repo가 오늘 trending됐다는 이유만으로 신작처럼 보이지 않게 한다.
...
공식 docs / leaderboard watch          85
Papers With Code / 구현 index           65
큐레이터 / aggregator                  65
...
Reddit / 저권위 social smoke            35
```

이 axis는 "GitHub trending 1위지만 사실 2년 전 repo가 단발 hype를 받는 것"과 "방금 nvidia가 release를 푸시한 것"을 정량적으로 구분한다. 트렌딩 1.0× / 1차 발신 1.6× 효과.

**교차 소스 승격**: DOI/arXiv/OpenReview/GitHub/HF/model 식별자와 정규화 title fallback으로 `cross_confirm_key`를 만든다. 독립 entity 2곳 이상에서 같은 item을 보고하면 `cross_confirm_count`, `cross_confirm_sources` metadata와 ranking boost를 받는다. 단, 이것은 승격 신호일 뿐 우회권이 아니다: freshness, 원본 URL 무결성, 환각 검증은 그대로 적용된다.

**구조적 cap** (shortlist 단계): 3배수 pool에서 aggregator, Tier-D, source repetition, trending/search feed를 제한한다. 트렌딩/search 소스(`github_trending_*`, `github_search_ai_repos`, `hf_models_trending`, Reddit, 약한 social search)는 final writing 전에 cap된다. final editorial review에서 7일 초과 stale-trending item은 더 신선한 후보가 남아 있는 한 primary slot을 차지하지 못한다.
...
| 레이어 | 선택 | 이유 |
|---|---|---|
| Runtime | **Python 3.11+**, 모든 모듈 상단에 `from __future__ import annotations` | type hint·dataclass 기반, GitHub Actions runner에서 사용 가능 |
| Orchestrator | **GitHub Actions + Codex auth** | cron 구동, 매 실행마다 fresh repo clone, 네이티브 `git push` 권한, KST 슬롯 guard |
...
| 학술 API | arXiv, Semantic Scholar, OpenAlex, Papers With Code | 넓은 학술 커버리지. optional key는 quota 개선용이고 baseline에는 필수 아님 |
| Delivery | **Telegram Bot API** (HTML mode, sendMessage) + **Notion API** (`notion-client` ≥ 2.2) | Telegram = push, Notion = 선택적 archive — 둘 다 무료 |
| Brief 작성 | **Codex** — affinity 점수 + 산문 작성 | LLM이 큐레이션 판단을 담당, 결정론적 Python이 그것을 감쌈 |
...
벡터 DB 없음. 외부 점수화 서비스 없음. 의도적으로 가볍고 작다. 예정된 작성 실행에는 Codex auth를 쓰고, 수집/전송 credential은 repository secret에 둔다.
...
| 학술 API quota / timeout | Semantic Scholar, OpenAlex, Papers With Code는 source-level warning 후 fail-open. 다른 학술 소스는 계속. |
| Reddit 429 / RSS 차단 | Reddit smoke source는 `[]` 반환 + cap 유지. 슬롯 송출의 필수 조건이 아님. |
| OpenReview API challenge | API access를 가정하지 않고 `vercel_watch`로 venue page를 감시. |
| 환각 claim 위험 | Step 8.5가 load-bearing claim 원문 URL을 fetch해 검증. 검증 실패 claim은 제거/완화하거나 item 자체를 교체. |
| 묵은 topic 누수 | `postcheck.py`가 시간 표기 누락과 slot window 위반을 reject. Codex가 publish 전에 rewrite/replace/drop. |
...
| YouTube RSS 불안정 | `youtube_ai`는 feed endpoint 안정화 전까지 비활성. 매 슬롯 noisy failure 없음. |
| Codex auth 부재 | workflow가 `CODEX_ACCESS_TOKEN` 또는 `CODEX_AUTH_JSON`를 작성 단계 전에 확인하고, 약한 브리프를 조용히 보내지 않고 명시 실패. |
| GitHub Actions 조기/지연 trigger | workflow가 KST slot을 계산하고 필요시 boundary까지 대기하며, 10:00 / 14:00 / 18:00 / 22:00 KST에만 publish. |
| GitHub Actions repo race | `state/seen.json`은 `main`에 push; `push_brief_stream.sh`가 `origin/claude/brief-stream` tip 위 temp index를 만들고 fast-forward push, 동시 update 시 retry. |
...
| env 변수 부재 (TELEGRAM_*) | Delivery가 warning + exit 0 (workflow를 죽이지 않음). |
...
라이브: PRISMA-Codex가 매일 KST 10:00 / 14:00 / 18:00 / 22:00에 실행된다. 최근 브리프는 card/detail 포맷, `hallu_*` diagnostics, 웹사이트용 `TC-SCORES` footer, `by Codex` 생성 footer를 모두 포함한다.
...
라이브 운영 중. 매일 KST 10:00 / 14:00 / 18:00 / 22:00에 PRISMA-Codex가 실행되어 위 KakaoTalk 오픈채팅으로 송출됩니다.
...
| 날짜 | 노트 | 한 줄 요약 |
|---|---|---|
| 2026-07-07 | [`releases/2026-07-07.ko.md`](./releases/2026-07-07.ko.md) · [English](./releases/2026-07-07.md) | PRISMA-Codex 전환 - GitHub Actions + Codex auth, 실효 enabled 채널 184개, 학술/소스 대확장, 교차 소스 확인, 정시 슬롯 guard 반영 |
