...
Claude Code routine이 매 슬롯 전체 파이프라인을 서버 사이드에서 실행한다 — 운영자의 컴퓨터는 꺼져 있어도 된다. 여섯 단계, 처음부터 끝까지: fetch · deduplicate · enrich · score · write · deliver.

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="./assets/architecture-dark.svg">
    <img alt="Trendchaser 시스템 아키텍처 — 약 70개 소스 채널을 fetch·deduplicate·enrich·score·write·deliver 6단계로 브리프로 만들고, 두 갈래 Git 구조에 저장한 뒤 Telegram·Notion·Obsidian 볼트로 분기하고, 마지막에 Telegram에서 공개 카카오톡 오픈채팅으로 중계한다." src="./assets/architecture.svg" width="100%">
  </picture>
</p>

| # | 단계 | 하는 일 |
|--:|---|---|
| 1 | **Fetch** | 약 70개 소스 채널을 병렬 폴링; 각 소스 best-effort, 장애 격리 |
| 2 | **Deduplicate** | 3단 — 14일 영구 상태, batch 내 소스 간, 브리프 history 스캔 |
| 3 | **Enrich** | 상위 ~30개 후보 본문 추출; 트렌딩 소스엔 novelty multiplier |
| 4 | **Score** | `profile.md` 기준 6축 합성 — signal·affinity·recency·novelty·velocity·freshness |
| 5 | **Write** | 슬롯별 선정 + 뉴스 브리프 산문, 소스 다양성 가드 |
| 6 | **Deliver** | Telegram push → KakaoTalk relay; Notion·Obsidian 아카이브 |
...
이후 **Shortlist 단계**에서 트렌딩 소스 cap을 강제: AI 섹션 최대 2개, General 섹션 최대 1개. "오래 누적된 trending이 brief를 잠식"하는 패턴을 구조적으로 차단한다.
...
routine 안 Claude가 점수화 단계에서 항목별로 계산. **2026-05-04 개정으로 novelty axis 도입한 6-axis 시스템**:
...
**구조적 cap** (shortlist 단계): 트렌딩 소스(`github_trending_*`, `hf_models_trending`)는 AI 섹션 2개 / General 섹션 1개로 hard cap. 브리프 작성 단계 이전에 강제되므로 "오래된 trending이 brief를 잠식"하는 패턴이 코드 단계에서 차단된다.
...
Telegram outbound 메시지에서는 `## 📌` 예고 섹션과 diagnostics 푸터가 strip된다. 브리프 파일과 Notion archive는 history 보존을 위해 그대로 유지.
...
| Scenario B — fetch/dedupe/enrich/deliver chain | 각 step `exit 0` |
...
| delivery self-test 스위트 | **14/14 pass** (escape, underscore 링크, chunk paragraph split, hard split, notion blocks, env-missing exit 0) |
